...
export async function POST(req: NextRequest) {
  try {
    ...
    const existingPlayer = await Round1Player.findOne({ teamId: team._id });

    if (existingPlayer) {
      return NextResponse.json(
        { error: "Round 1 already started. Please contact event organizer" },
        { status: 403 }
      );
    }

    const round1player = await Round1Player.create({
      teamId: team._id,
      startTime: new Date(),
      score: null,
      timeTaken: null,
